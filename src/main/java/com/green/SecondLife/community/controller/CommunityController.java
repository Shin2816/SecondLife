...
public class CommunityController {
    ...
    public String freeBoardList(Model model, BoardFreeListVO boardFreeListVO, SubMenuVO subMenuVO){
        //페이지 정보 세팅
        int totalDataCnt = communityService.selectBoardCnt(boardFreeListVO); //전체 게시글 갯수 조회해서
...
//    }
    ...
    public String boardDetail(Model model, SubMenuVO subMenuVO, BoardCommentListVO boardCommentListVO){
        ...
        //조회수 증가
        communityService.updateFreeBoardCnt(boardCommentListVO.getCommentNum());
        //댓글 조회해서 html로 던지기
        model.addAttribute("comment", communityService.selectFreeBoardComment(boardCommentListVO.getCommentNum()));
    ...
    public String freeBoardComment(BoardCommentListVO boardCommentListVO, RedirectAttributes redirectAttributes){
        communityService.insertFreeBoardComment(boardCommentListVO);

        return "redirect:/board/boardDetail?menuCode="+ConstantVariable.MENU_CODE_BOARD+ "&commentNum=" + boardCommentListVO.getCommentNum(); //디테일 컨트롤러로 다시 이동할 때 값 가져가기
    }

    //상세 페이지에서 댓글 삭제버튼 클릭하면 delete 쿼리 실행
    @PostMapping("/freeDeleteComment")
    public String freeDeleteComment(BoardCommentListVO boardCommentListVO, RedirectAttributes redirectAttributes){
        communityService.deleteFreeBoardComment(boardCommentListVO.getCommentId());//삭제 쿼리
    ...
    public String freeDeleteComment(BoardCommentListVO boardCommentListVO){
